...
$large_image_name = $imgsrc; 		// New name of the large image
$thumb_image_name = "th_".$imgsrc; 	// New name of the thumbnail image
...
$path1 = $upload_path."/".$getid.".".$file_ext;
$path1a = $upload_path."/".$getid.".jpg";
$path1b = $upload_path."/w_".$getid.".jpg";
move_uploaded_file($userfile_tmp, $path1);
if ($file_ext == "png") 
{
...
<title>E=mc3 Member Picture</title>
...
<body style="background-image:none; background-color:#FFFFFF; padding:10px">
<?php
...
if(strlen(isset($error))>0){
	echo "<div class='success'>Error!".$error."</div>";
}
if(strlen($large_photo_exists)>0 && strlen($thumb_photo_exists)>0){
	...
	echo "<p><a href=\"".$_SERVER["PHP_SELF"]."?a=delete&id=".$getid."\">Delete Picture</a></p>";
}else{
		if(strlen($large_photo_exists)>0){?>
		...
		<div align="center">
			...
			<br style="clear:both;"/>
			<form name="thumbnail" action="<?php echo $pagename;?>?id=<?php echo $getid;?>" method="post">
	...
	<hr />
	<?php 	} ?>
	<h2>Upload Photo</h2><br />Upload Horizontal Image in jpg format.<br /><br />
	<form name="photo" enctype="multipart/form-data" action="<?php echo $pagename;?>?action=upload&id=<?php echo $getid;?>" method="post">
...
<?php } ?></td>
                  ...
                  <tr>
                    <td  align="center"><?php
if($action1=="success") {?> <div class="success">Deleted Picture</div>
                    <?php }?>
<?php
if($action1=="err") { ?> <div class="error">something error</div><?php }?></td>
                  </tr>
